...
numéricas asi como en el sexo de los pinguinos. Esto es importante ya que si hubiera una gran diferencia
...
Tampoco creemos necesario mantener el valor de X, ya que es un identificador que no aporta información
...
data_Num <- model.matrix(~ . -1, data = data_NoX)
...
Como se puede ver, las 2 componentes principales explican el 85% de la varianza de los datos. Esto nos
...
que perjudique a los propios clusters. Veamos a ver si se pueden sacar conclusiones con algunas variables.
...
las muestras. Además se puede apreciar una tendencia de que el clúster 1 forman parte del grupo de hembras
mas pesadas, mientras que el clúster 4 forman parte del grupo de hembras más ligeras. En el caso de los
...
# Normalize the data
data_Normalized <- as.data.frame(lapply(data[2:5], normalize))
data_Normalized <-cbind(data_Normalized, data[,6])
...
idx <- sample(1:dim(data_Normalized)[1], 40)  # Seleccionar 40 muestras aleatorias
...
data_Sample.pca <- prcomp(data_Sample[, 1:4], center = TRUE, scale = TRUE)

# Obtener las dos primeras componentes principales
data_Sample_PCA <- as.data.frame(data_Sample.pca$x[, 1:2])

# Añadir los clusters al dataframe de PCA desde el dataframe original
data_Sample_PCA$cluster <- as.factor(data_Sample$hc.Clusters)

# Visualizar los clusters usando ggplot
ggplot(data_Sample_PCA, aes(x = PC1, y = PC2, color = cluster)) +
  geom_point(size = 3) +
  labs(title = "Clusters del Clustering Jerárquico", x = "PC1", y = "PC2") +
  theme_minimal()
```
