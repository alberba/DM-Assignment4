...
#distEuclideaSample <- dist(data_Sample[, 1:6], method = "manhattan")
```
...
data_PCA <- as.data.frame(dataNoSex_Scaled.pca$x[,1:2])
data_PCA <- as.data.table(data_PCA)

# Calcular la distancia euclidea
distEuclidea <- dist(dataNoSex, method = "euclidean")
#distEuclidea <- dist(dataNoSex, method = "manhattan")
```
...
#distEuclideaSample <- dist(data_Sample[, 1:4], method = "manhattan")
...
rect.hclust(hclust_single2, k=4)
rect.hclust(hclust_single2, k=3)
```
...
```{r}
groups <- cutree(hclust_average, k = 3)
...
```{r}
groups <- cutree(hclust_complete, k = 3)
