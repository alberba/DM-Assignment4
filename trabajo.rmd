...
In this case, the largest jump occurs in step 331, so it is determined that the optimal number of clusters
seems to be 3 using this method.
...
the penguins grow up, which is simply ilogical.
...
El objetivo de este proyecto fue buscar maneras de poder agrupar el conjunto de pingüinos de una manera
coherente a partir de clústers.

Mientras explorabamos los datos, nos encontramos con que había varios valores nulos que eliminamos ya que no
suponían un porcentaje significativo del dataset. También eliminamos las columnas de X (identificador del
pingüino) y year, ya que no aportaban información relevante para el análisis y vimos que entre las variables
faltantes había una fuerte correlación, por lo que decidimos aplicar PCA para reducir la dimensionalidad de 
los datos.

Una vez aplicamos el PCA, transformamos el dataset para convertirlo en valores numéricos. Para los modelos
particionales, escalamos el dataset y en el caso de los hierarchical clustering, normalizamos el dataset.

Una vez transformamos el dataset, aplicamos los diferentes modelos de clustering. En el caso de los modelos
particionales, aplicamos el k-means, el PAM y el DBSCAN, mientras que en el caso de los hierarchical 
clustering, aplicamos los diferentes métodos de linkage. En cada uno de los modelos, a partir de unas 
funciones, definimos el número de clusters óptimo.

En cuanto a los resultados, vimos que tanto el k-means, el PAM, el DBSCAN y algunos linkage del hierarchical 
clustering, nos daban resultados muy similares, por lo que no podemos decir que haya un mejor modelo.

A la hora de interpretar los clústers, nos fijamos como cada sexo tenía 2 clústers, lo cual nos indicaba que
el sexo del pingüino tuvo un papel importante en la agrupación. Vimos como parecía que los clústers 
agrupaban los pingüinos según si eran adultos o jóvenes, pero al mirar las variables, vimos que no tenía
mucho sentido.

Por último, aplicamos los mismos modelos pero eliminando la variable de sexo, ya que vimos que influenciaba
mucho en la agrupación. Los resultados de este dataset fueron muy similares a los anteriores, por lo que no
pudimos sacar ninguna conclusión adicional. La suposición final de como se agrupaban los pingüinos fue que
se agrupaban según especies.

### Our Learnings

Hemos aprendido lo complicado que es llegar a un veredicto final en cuanto a la agrupación de los datos, pero
también hemos aprendido a aplicar los diferentes modelos de clustering que existen y sobretodo, a interpretar
los resultados según el dataset y sus variables.
